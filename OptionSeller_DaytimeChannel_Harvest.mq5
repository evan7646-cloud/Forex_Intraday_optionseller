...
#property version   "3.00" // 策略版本號 (白天全天通道專屬版)
...
input int      InpEndHour             = 18;     // 開倉結束小時 (MT5 18:00 = 台北 23:00)
...
{ // 區塊開始
   ...
   if(dt.hour >= InpStartHour && dt.hour <= InpEndHour) return true; // 符合白天窗口
   return false; // 否則禁止
...
{ // 區塊開始
   ...
   { // 開倉檢查
      ...
      { // 做多
         ...
         m_trade.Buy(InpLotSize, _Symbol, ask, sl, prev_bb_mid, "DayChannel_ShortPut_Buy"); // 下多單
      } // 結束
      ...
      { // 做空
         ...
         m_trade.Sell(InpLotSize, _Symbol, bid, sl, prev_bb_mid, "DayChannel_ShortCall_Sell"); // 下空單
      } // 結束
   } // 開倉結束
