...
#property description "🌙 策略二【晚間美盤午後極窄點差收租 EA】：專攻 GBPJPY (1h), GBPUSD (15m), EURUSD (15m), EURNZD (1h), EURJPY (1h)，運作時段 MT5 13:00~19:00 (台北 18:00~00:00)" // 描述
...
input bool     InpAutoParams          = true;   // 啟用自動品種參數適配 (EURNZD/EURJPY 2.5σ, GBPJPY/GBPUSD 2.2σ, EURUSD 2.0σ)
...
{ // 區塊開始
   ...
   if(clean == "EURNZD" || clean == "EURJPY") return 2.5; // EURNZD/EURJPY 美盤最佳為 2.5σ
   if(clean == "GBPJPY" || clean == "GBPUSD") return 2.2; // GBPJPY/GBPUSD 最佳為 2.2σ
   if(clean == "EURUSD") return 2.0; // EURUSD 最佳為 2.0σ
   return 2.2; // 預設 2.2σ
} // GetOptimalSigma 結束
...
{ // 區塊開始
   ...
   string clean = CleanSymbolName(sym); // [P1 修復] 清理 Symbol 後綴
   if(clean == "GBPUSD" || clean == "EURUSD") return 2.0; // GBPUSD/EURUSD 最佳為 2.0 ATR
   if(clean == "GBPJPY" || clean == "EURNZD" || clean == "EURJPY") return 1.5; // GBPJPY/EURNZD/EURJPY 最佳為 1.5 ATR
...
{ // 區塊開始
   ...
   Print("=========================================================================="); // 分隔線
   Print(" 🌙【策略二：晚間美盤午後極窄點差收租 EA v4.10】成功啟動！標的: ", _Symbol, " (清理後: ", CleanSymbolName(_Symbol), ") | 週期: ", EnumToString(_Period)); // 啟動提示
...
{ // 區塊開始
   ...
   Print("🔌 策略二【晚間美盤收租 EA v4.10】已卸載。"); // 輸出日誌
} // OnDeinit 結束
