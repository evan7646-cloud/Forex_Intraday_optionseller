...
#property version   "3.00" // 策略版本號 (美盤午後專屬版)
#property description "🌙 策略二【晚間美盤午後極窄點差收租 EA】：專攻 GBPJPY (1h), EURAUD (1h), GBPUSD (15m), EURUSD (15m), AUDUSD (15m)，運作時段 MT5 13:00~19:00 (台北 18:00~00:00)" // 描述
...
input double   InpLotSize             = 1.0;    // 交易下單手數 (固定 1.0 手)
...
input bool     InpAutoParams          = true;   // 啟用自動品種參數適配 (EURAUD 2.8σ, GBPJPY/GBPUSD/AUDUSD 2.2σ, EURUSD 2.0σ)
...
{ // 區塊開始
   ...
   if(sym == "EURAUD") return 2.8; // EURAUD 最佳為 2.8σ
   if(sym == "GBPJPY" || sym == "GBPUSD" || sym == "AUDUSD") return 2.2; // 最佳為 2.2σ
...
{ // 區塊開始
   ...
   if(sym == "AUDUSD") return 2.5; // 最佳為 2.5 ATR
   if(sym == "GBPUSD" || sym == "EURUSD") return 2.0; // 最佳為 2.0 ATR
...
{ // 區塊開始
   ...
   Print("=========================================================================="); // 分隔線
   Print(" 🌙【策略二：晚間美盤午後極窄點差收租 EA】成功啟動！標的: ", _Symbol, " | 週期: ", EnumToString(_Period)); // 啟動提示
...
{ // 區塊開始
   ...
   Print("🔌 策略二【晚間美盤收租 EA】已卸載。"); // 輸出日誌
} // OnDeinit 結束

//+------------------------------------------------------------------+ // 函數分隔
//| 檢查當前是否處於美盤允許開倉時間窗口                                    | // 函數說明
//+------------------------------------------------------------------+ // 分隔線
...
{ // 區塊開始
   ...
   MqlDateTime dt; // 宣告時間結構
   TimeToStruct(TimeCurrent(), dt); // 解析當前時間
   if(dt.hour >= InpForceCloseHour || dt.hour == 0) return true; // MT5 22:00 後強制全平
...
{ // 區塊開始
   ...
   double closed_price = rates[0].close; // 前一根收盤
   ...
   { // 遍歷
      ...
      { // 成功
         ...
         { // 符合
            ...
            { // 多單
               if(InpExitAtBBMiddle && closed_price >= prev_bb_mid && closed_price > m_position.PriceOpen()) // 碰中軌止盈
               { m_trade.PositionClose(m_position.Ticket()); has_position = false; } // 平倉
            } // 結束
   ...
   { // 開倉檢查
      ...
      if(closed_price <= prev_bb_lower && prev_rsi <= InpRSI_Oversold) // 多單訊號
      { // 做多
         double ask = m_symbol.Ask(); // 買價
         double sl = ask - (active_atr_sl * prev_atr); // 止損價
         m_trade.Buy(InpLotSize, _Symbol, ask, sl, prev_bb_mid, "US_Afternoon_ShortPut_Buy"); // 下多單
      } // 結束
      else if(closed_price >= prev_bb_upper && prev_rsi >= InpRSI_Overbought) // 空單訊號
      { // 做空
         double bid = m_symbol.Bid(); // 賣價
         double sl = bid + (active_atr_sl * prev_atr); // 止損價
         m_trade.Sell(InpLotSize, _Symbol, bid, sl, prev_bb_mid, "US_Afternoon_ShortCall_Sell"); // 下空單
      } // 結束
   } // 開倉結束
